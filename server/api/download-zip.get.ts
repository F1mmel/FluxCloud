...
import { defineEventHandler, getQuery, setResponseHeaders, sendStream, createError } from 'h3'
...
export default defineEventHandler(async (event) => {
  ...
  const archiveName = (query.zipName as string || 'fluxcloud_download').replace(/[\\/:\*\?"<>\|]/g, '_')
  
  let relativePaths: string[] = []
  if (Array.isArray(query.path)) {
    relativePaths = query.path as string[]
  } else if (typeof query.path === 'string') {
    relativePaths = [query.path]
  } else if (typeof query.paths === 'string') {
    try {
      const parsed = JSON.parse(query.paths)
      if (Array.isArray(parsed)) relativePaths = parsed
      else relativePaths = [query.paths]
    } catch {
      relativePaths = query.paths.split(',').map(s => s.trim()).filter(Boolean)
    }
  }
