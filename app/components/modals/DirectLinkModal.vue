      ...
      >
        ...
        <div class="p-6 space-y-5">
          ...
          <div v-else class="space-y-5">
            <!-- 1. Direct URL Bar + Copy Button on top -->
            <div class="space-y-2">
              <div class="flex items-center justify-between">
                <label class="text-xs font-semibold text-[#0f172a] dark:text-[#fafafa] flex items-center gap-1.5">
                  ...
                  <span>{{ isDirectDownload ? 'Direct Auto-Download URL' : 'Direct Raw CDN Stream URL' }}</span>
                </label>
                <span class="text-[10px] font-mono text-[#64748b] dark:text-[#cbd5e1]">
                  {{ isDirectDownload ? 'Auto-Download Trigger 🚀' : 'Raw Byte Stream ⚡' }}
                </span>
              </div>
            ...
            <div 
              class="p-3.5 rounded-2xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 flex items-center justify-between gap-3 cursor-pointer hover:bg-black/[0.07] dark:hover:bg-white/[0.07] transition-all" 
            ...
            >
              <div class="flex items-center gap-2.5 min-w-0">
                ...
                <div class="flex flex-col min-w-0">
                  <span class="text-xs font-bold text-[#0f172a] dark:text-[#fafafa]">Direct Auto-Download</span>
                  <span class="text-[11px] text-[#64748b] dark:text-[#cbd5e1]">Starts download immediately when accessing this URL (?download=1)</span>
                </div>
              </div>
            ...
            <div class="pt-3 border-t border-black/5 dark:border-white/10 flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col">
                  ...
                  <span class="text-[10px] text-[#64748b] dark:text-[#cbd5e1]">Scan to open direct stream</span>
                </div>
              </div>
...
const copyActiveUrl = async () => {
  if (await copyToClipboard(activeUrl.value)) {
    const msg = isDirectDownload.value ? 'Direct Auto-Download URL copied' : 'Direct raw CDN link copied'
