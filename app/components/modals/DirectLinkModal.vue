      ...
      >
        ...
        <div class="p-6 space-y-5">
          ...
          <div v-if="loading" class="flex flex-col items-center justify-center py-12 gap-3">
            ...
            <span class="text-xs text-[#64748b] dark:text-[#cbd5e1]">Generating permanent share tokens...</span>
          </div>

          <div v-else class="space-y-5">
            <!-- 1. Primary Link: Public Share Landing Page (Default for sharing in browser) -->
            <div class="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/20 space-y-2.5">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs font-bold text-[#0f172a] dark:text-[#fafafa]">Public Share Landing Page</span>
                ...
                <span class="text-[10px] text-[#64748b] dark:text-[#cbd5e1]">PDF Reader • Preview • Download</span>
              </div>

              <div class="flex items-center gap-2">
                <input 
                  type="text" 
                  :value="fullPublicShareUrl" 
                  readonly 
                ...
                />
                <button 
                  @click="copyUrl(fullPublicShareUrl, 'Public Share Page URL copied!')" 
            ...
            <!-- 2. Secondary Link: Direct Raw CDN Stream URL (For Embedding / Hotlinking / Download) -->
            <div class="p-4 rounded-2xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 space-y-2.5">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs font-bold text-[#0f172a] dark:text-[#fafafa]">{{ isDirectDownload ? 'Direct Auto-Download Stream' : 'Direct Raw CDN Link' }}</span>
              ...
              <div class="flex items-center gap-2">
                ...
                />
                <button 
                  @click="copyUrl(activeCdnUrl, isDirectDownload ? 'Auto-Download URL copied!' : 'Direct Raw CDN link copied!')" 
            ...
            <div class="pt-3 border-t border-black/5 dark:border-white/10 flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col">
                  ...
                  <span class="text-[10px] text-[#64748b] dark:text-[#cbd5e1]">Scans to Public Share Landing Page</span>
                </div>
              </div>
...
const fullPublicShareUrl = computed(() => {
  if (typeof window === 'undefined' || !directData.value?.publicShareUrl) return ''
  return `${window.location.origin}${directData.value.publicShareUrl}`
})
...
const qrCodeUrl = computed(() => {
  return getQrCodeUrl(fullPublicShareUrl.value || fullDirectUrl.value)
})
...
const loadDirectToken = async () => {
  ...
  try {
    ...
    error('Direct Link Error', err?.data?.statusMessage || 'Failed to generate share tokens')
    emit('close')
  } finally {
    loading.value = false
  }
